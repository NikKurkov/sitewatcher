# sitewatcher — мониторинг сайтов и доменов + Telegram-бот
...
**Проверки (checks):**
- **HTTP Basic** — статус-код и время ответа главной страницы (с учётом редиректов).
- **TLS-сертификат** — срок действия, предупреждение за N дней до истечения.
- **Ping** — ICMP-пинг (на Linux/macOS; на Windows может быть недоступен).
- **Keywords** — наличие обязательных ключевых слов/строк в HTML (например, счётчик/метрика).
- **RKN Block** — проверка на блокировки в реестрах (офлайн-индекс, обновляемый по расписанию).
- **IP change** — отслеживание смены A-записей домена.
- **IP blacklist** — проверка IP на наличие в чёрных списках (если включено).
- **Ports** — проверка доступности заданных портов.

**Другие функции:**
- Планировщик периодических проверок (per-check интервалы по умолчанию + доменные override).
- Разовые «эфемерные» проверки домена **без** записи в БД и **без** алертов.
- Дедупликация и «охлаждение» алертов, чтобы не спамить при повторяющихся проблемах.
- Ограничение доступа к боту по списку Telegram-ID.
- SQLite-хранилище, создаётся автоматически.
...
```bash
# 1) Требования
# Python 3.11+ (рекомендуется 3.12), Linux/macOS для ICMP ping
...
- `/start` / `/help` — справка.
- `/add_domain <name.ru> [ещё.ru ...]` — добавить один или несколько доменов.
- `/remove_domain <name.ru>` — удалить домен.
- `/list_domain` — показать домены владельца.
- `/check_domain <name.ru>` — проверить домен разово **без** записи в историю и **без** алертов (для доменов, которых нет в БД). Для уже добавленных — обычная проверка.
...
Файл `config.yaml` позволяет переопределить дефолты и создать доменные профили. Укажите путь через `--config ./config.yaml`.
...
defaults:
  ...
  tls_warn_days: 21
  proxy: null              # прокси для HTTP-проверок (строка URL) или null
  keywords: []             # глобальный список ключевых слов (можно переопределять на домене)
  checks:
    http_basic: true
    tls_cert:   true
    ping:       true
...
- Для «разовой диагностики» неизвестного домена используйте `/check_domain`: это не изменит БД и не отправит алерт.
...
Рекомендуется добавить `pre-commit` (ruff/black/isort/pyupgrade) и CI (ruff + pyright + pytest).

---

## Лицензия

MIT (при необходимости укажи в репозитории).
